...
class GroupNanNorm(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(
            (int(shape[0]), self.num_groups, int(shape[1]) // self.num_groups, -1)
        )
...
class ResidialBlock1D(torch.nn.Module):
    def __init__(
        ...
        if nangroup:
            self.norm = GroupNanNorm(
                num_groups=output_size // group_size,
